...
web.o.card_ev = function (container_id) {
    ...
    o.gen_html = function () {
        return '<div class="card-div">' +
                '<input type="button" id="' + o.ids[0] + '" value="显示时间" class="card-btn">' +
                '<input type="button" id="' + o.ids[1] + '" value="停止" class="card-btn">' +
                '<div class="card-text" id="' + o.ids[2] + '"></div>' +
                '</div>';
    };

    // 创建一个显示当前时间的函数
    o.update = function () {
        o.objs[2].innerHTML = (new Date()).toLocaleString();
...
web.o.card_fetch = function (container_id) {
    ...
    o.gen_html = function () {
        return '<div class="card-div" style="overflow:auto;">' +
                '<input type="button" id="' + o.ids[0] + '" value="读取数据" class="card-btn" style="float:left;">' +
                '<div style="float:left;" id="' + o.ids[1] + '">*需php支持*</div>' +
                '</div>';
    };
    ...
    web.o.cboard = function (container_id) {
            ...
            cb.gen_html = function () {
                ...
                for (var i = 0; i < 3; ++i) {
                    html += '<tr>';
                    for (var j = 0; j < 3; ++j) {
                        html += '<td><input type="button" id="' + cb.ids[i * 3 + j] + '" class="chess" title="点击下棋"></td>';
                    }
                    html += '</tr>';
                }
                html += '  </table></td>' +
